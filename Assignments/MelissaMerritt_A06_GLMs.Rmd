...
# In order to get only the records we need, we use the pipe function (%>%). Filter() allows me to select on the day numbers that are within July. Select() allows me to only keep the columns that are relevant. Drop_na() allows me to get rid of the noncomplete cases. 
...
> Answer: The figure suggests that as depth increases, temperature decreases. The linerarity of the points suggests that there were monitors at specific depths, and the shallower depths have the most variability in temperature as compared to deeper temperature recordings. 
...
corrplot.mixed(NTL_LTER_July_predict_corr, upper = "ellipse")

# the corrplot.mixed() function allows us to see both the ellipses and the NUMBER?? (what is this number?) Both of these plots will give us a better indication of which variables should be included in the regression. 
...
# The year variable does not have a strong correlation, so we can remove this variable from the next regression. (it is still recommended)


#10

temp.multiple.regression <- lm(data = NTL_LTER_July_predict, temperature_C ~ depth + daynum)
