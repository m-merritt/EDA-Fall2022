...
withdrawal_df <- data.frame("Month" = c("Jan","May","Sept","Feb","Jun","Oct","Mar","Jul","Nov","Apr","Aug","Dec"),
                            "Year" = rep(2021),
                            "Water.System" = rep(water.system.name), 
                            "PSWID" = rep(pswid), 
                            "Ownership" = rep(ownership), 
                            "Maximum.Daily.Use.mgd" = as.numeric(max.withdrawals.mgd) %>%
  mutate(Date = my(paste(Month,"-",Year)))
                            
# To create the data frame with these values, we can use the data.frame() function. Within this function, we can set the column names in the quotation marks, and set them equal to the scraped values. We can also make a month and year column using the rep() function to repeat the values, and for the month column we can manually add each of the months in to match the max.withdrawals. The last column we add is the month-year column using the mutate() function and the lubridate package my() function.  
