...
# Using the pipe function (%>%) I was able to mutate() and add the year and month columns using the year() and month() functions. Then we can use the group_by() to specify that we want to use the rows that have the same values in month and year column for the means. We use the summarise() function to then get the mean. 
...
```{r}
#10
GaringerOzone.daily.ts <- ts(GaringerOzone$Daily.Max.8.hour.Ozone.Concentration, start = c(2010,1), end = c(2020,12), frequency = 365)
  
GaringerOzone.monthly.ts <- ts(GaringerOzone.monthly$Mean_ozone_concentration, start = c(2010,1), end = c(2019,12), frequency = 12)
