...
# we use ggplot because it has more functionality and flexibility to make it look the way you want it 
# it is very easy to generate plots - its hard to generate meaningful plots 

#this is how we can put text into the plot 
# paste - pasting text together 
...
practice_plot1 <- ggplot(subset(PeterPaul.chem.nutrients, depth == 0), aes(x = daynum, y = temperature_C)) + 
                           geom_point(aes(color=year4)) +
   facet_wrap(vars(lakename), nrow = 2) +
  ylab("surface temperature") +
xlab("date")
print(practice_plot1)
...
practice_plot2 <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C, color=depth) +
  geom_point(aes(color=depth), size = 0.5) +
    ylab("temperature in celsius") +
    scale_color_viridis(option = "magma", direction = -1) +
    scale_x_date(date_breaks="5 years", date_labels="%Y")
print(practice_plot2) 
...
ggplot(EPAair, aes(x = PM2.5, y = Ozone)) + 
  geom_point(aes(color = Mean_latitude), alpha = 0.5) + 
