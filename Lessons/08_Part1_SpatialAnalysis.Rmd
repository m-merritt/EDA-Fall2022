...
#downloaded directly from the website. Geometry column has spatial points 
...
#puts the points in the map view
...
#create a table as a simple feature. coordinate reference systems (crs). 
...
#it doesn't know where to put this point on the map, so we have to link it with the crs. 
...
#adding attributes to the point. This is how you might construct a spatial data frame from scratch. 
...
lat/long are useful for location, but not for finding distances between, so we need to reproject these coordinates. 
...
# the one refers to 1 degree, so we need to reconfigure this circle so it maps well on a plane. 
...
UTM - x and y values are in meters 
...
#st_distance() - pairwise means we want to create a matrix
