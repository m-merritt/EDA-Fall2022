...
Unethical if the website has clear instruction saying it is "disallowed"
...
#the_registrant is extracted from the webpage - then we will go and read bits from that webpage. html_nodes is saying to go to this webpage item (tag and value pair) and look for all the elements that have these () descriptors. then pipe it to htm_text() so we get the key value text associated with the node. 
```
...
#get these through selector gadget on chrome. These are character vectors, so we will have to convert to numbers later on
...
#Create a dataframe of withdrawals (tidy) 
...
#convert withdrawals to numeric values, and show how to identify by month and year
...
# != bangs (the bangs mean this is referring to a variable) and this is setting the month and years to dates
...
# if you set things as variables you can control what you are veiwing by just changes the variable in your code chunk

#converting a lot of inputs into variables. basically built the website that we were trying to scrape 
...
scrape.it <- function(the_year, the_facility){
  ...
  #tells R to just wait for one second. this is because the server might feel under attacked if you and might limit it. It could cut off access, and they might limit dukes access to certain website (the function about is for etquiette and to avoid this). five seconds might be better
...
# above is saying to repeat this function for every year in the vector (specify the facility). 
...
#not the best plot (don't use for HW)
```
...
# iterating between the 20 different facilities 
...
# you could look at by month by using x = as.factor(month). 
