...
Temp_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% # using the space to separate 
  mutate( Date = my(paste0(Month,"-",Year))) %>% 
...
Rain_plot <- ggplot(Rainfall_BR_processed, aes(x = Date, y = Rainfall)) +
  geom_line() +
...
Temp_plot <- ggplot(Temp_BR_processed, aes(x = Date, y = Temperature_C)) + 
  geom_line() + 
  geom_smooth(method = lm, color = "blue", se = F)
print(Temp_plot)
...
# Rainfall seems like a straight line (slope = 0 or stationary) temperature is more noticeably increasing over time.
...
Rain_TS <- ts(Rainfall_BR_processed$Rainfall, start = c(1901,1), frequency = 12)
Rain_TS
?ts()  
...
#seasonal component is explaining the most of the variability of the data set for rain 
#Both seasonal and trend are having the same impact on our dataset and remainder (but we don't like remainder) (look at the bars on the right side to see which have the most impact on the variability)
...
Rain_components <- mutate(Rain_components, Observed = BRA_complete$Rainfall,
