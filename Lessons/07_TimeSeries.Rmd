...
<<<<<<< HEAD
this dataset have different time steps - this will show us the difference in trend projection. 
...
<<<<<<< HEAD
# create two additional datasets - store early and later observations since that is the way the data is separated. This allows us to perform separate analysis 
...
```{r}
<<<<<<< HEAD
# stl() function only accepts time series objects, so need to transform into a time series. Choose the column that you want to analyze, in this case the discharge mean. You can either choose column 8, or do the "$" which is shown below. Important to define the starting point. in this case, january 1985, frequency equals 365, so this is how it will know it is a time series object. 

EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365)

 head(EnoDischarge.late_ts, 10)
 # allows you to see the results of the time series. whatever number you put at the end (example: "10") shows the first results 
=======
EnoDischarge.late_ts <- ts(EnoDischarge.late$discharge.mean, start = c(1985,1), frequency = 365)
>>>>>>> fc8c57328ec11d4acfebcceb37adf859baecba8b
```
...
?stl
<<<<<<< HEAD
#this gives us information about this function (use this for the homework questions)
# Generate the decomposition
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic")

# x argument - univariate time series to be decomposed (do not use two variables)
# s.window argument refers to the seasonal window and you can define it as periodic 

# Visualize the decomposed series. 
plot(EnoDischarge.late_Decomposed)

#the season compenent repeats itself every 365 observations, the trend is not monotonic because it is sometimes increasing and sometimes decreasing 
...
#above creating a data frame, object for time series has seasonal, trend and remainder. We will create a data frame with these three columns 
...
<<<<<<< HEAD
#above adding two more columns adding the date object and observed. this is so we can make the plots 
...
<<<<<<< HEAD
#add a line for observed and trend components. Add a special color to make the trend pop out. Line at the intercept to call information to the fact that we don't have values less than zero, but some might be 0. 
...
<<<<<<< HEAD
# this plot shows the seasonal changes in the same color as the trend line. Seasonal component goes below zero
=======
>>>>>>> fc8c57328ec11d4acfebcceb37adf859baecba8b
```
...
<<<<<<< HEAD
# you can see that there is one NA observation

# Adding new column with no missing obs, just for illustration purpose
# In real applications you will simply replace NAs
# = zoo::na.approx(AWND) - you need to add zoo because it is the name of the package - this means you don't need to library the entire package. na.approx is a function to clean the dataset 
=======
# Adding new column with no missing obs, just for illustration purpose
# In real applications you will simply replace NAs
>>>>>>> fc8c57328ec11d4acfebcceb37adf859baecba8b
...
<<<<<<< HEAD
#Note the NA is gone and the mean, median and max, did not change - interpolation won't change main properties of your distribution. 
=======
#Note the NA is gone
>>>>>>> fc8c57328ec11d4acfebcceb37adf859baecba8b
...
#red segment in line represents the replacement (YOU CANNOT JUST REMOVE NAs unless they are at the beginning or end of the dataset)
=======
>>>>>>> fc8c57328ec11d4acfebcceb37adf859baecba8b
```
...
#defining first month and first year can be really helpful with TSA
=======
>>>>>>> fc8c57328ec11d4acfebcceb37adf859baecba8b
...
<<<<<<< HEAD
#this shows us the seasonal component (much easier and more stable). Trend is going up and down, but overall trend of decrease (still need a statistical test to show with numbers)
...
<<<<<<< HEAD
# results are pointing to a p-value that is less than 0.05, so we would reject the null hypothesis to say we have a trend. 
=======
>>>>>>> fc8c57328ec11d4acfebcceb37adf859baecba8b
