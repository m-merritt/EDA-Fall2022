...
#"skip = " skip the first 29 lines. "sep='\t'" to ensure it is tab separated data. 
...
#changed the site number and the period in the url. this is the only change needed. 

theURL_2 <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

gage_data <- read.table(theURL_2,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 
...
# one of the data retreival commands
...
#we would need to get our own key 
...
the_states <- 'tr :nth-child(1)'
the_price <- 'tr :nth-child(2)'
the_capacity <- 'tr :nth-child(3)'
net_generation <- 'tr :nth-child(4)'
total_retail <- 'tr :nth-child(5)'

the_states <- the_website %>% html_nodes(the_states) %>% html_text()
the_price <- the_website %>% html_nodes(the_price) %>% html_text()
the_capacity <- the_website %>% html_nodes(the_capacity) %>% html_text()
net_generation <- the_website %>% html_nodes(net_generation) %>% html_text()
total_retail <- the_website %>% html_nodes(total_retail) %>% html_text()
...
>>>>>>> 581725fcf5b8d194baf9083ad431b30b422f687e
  
  # above: for all years that aren't 2017, the year will look like this 
the_website <- read_html(the_url) #this is the change 
...
df_multiyear <- map(seq(2017,2019), scrape.it) %>% bind_rows()
