...
=======
PeterPaul.chem.nutrients <- read.csv("./Data_2022sp/Processed/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv", stringsAsFactors = TRUE)
...
<<<<<<< HEAD
# filter out zeros above, the summary shows the higher the depth the higher the irradiance. now with the log transformed data the r^2 increased a lot so this is a much better fit 
...
<<<<<<< HEAD
# summary shows the estimates for the intercept and the relationships between the multiple variables (p value less than .05) 
...
<<<<<<< HEAD
# above plot shows that the variables are based on depth 
...
#only have the columns of interest in the matrix - select()
# oval shapes represent the scatter plot of one variable against the other 
#dark red (-1) a strong but negative correlation 
# dark blue (+1) a strong but positive correlation
=======
>>>>>>> fc8c57328ec11d4acfebcceb37adf859baecba8b
```
...
<<<<<<< HEAD
#step() function gives results showing the starting point for the AIC and the other variables and new AIC values for when you remove other variables 

#best regression is considering oxygen, nitrogen and temperature

#usually using step and choosing variables lead to a stronger r^2 value
=======
>>>>>>> fc8c57328ec11d4acfebcceb37adf859baecba8b
```