...
#these are shapefiles. calculate mean and max for the particulate matters. st_as_sf() turns the values into spatial "data frame". 
...
# the filter is for just the specific states (NC). Heirarchy of cesus blocks (think of GIS). Since we have a geometry column it is a spatial data frame.  
...
st_bbox(counties_sf)
# this tells us the coordinates of upper right and lower left of bounding box
...
# you can view the maps by the different columns 
```
...
#uses WGS 84. Not the same as the counties. 
...
- a shape file written out as text. 
...
# filter for the records that are not photovoltaic

# Examine counts by fuel type
ggplot(nc_powerplants_sf) + 
  geom_sf(aes(color = PRIM_FUEL)) 

# not great map options with ggplot, but you can still map it with coordinates
...
# the shape file knows FIPS are not numbers - important because we need it to keep the preceding zero - it is important to make sure you don't bring these codes in as numbers. Stringsasfactor does not work. 
...
#same join just tidyverse way to do it
...
#shows us the poverty in counties along with pollution. 
...
# use the HUC 12 to find the HUC 8 (using the numbers 1 through 8)
...
#Select intersecting counties using the `filter()` command - tidy verse
...
#adding the area as numeric values. 
...
#like putting a rubber band around the shape. 
...
#good for interpolation (defining areas closest to a given point)
...
#if we wanted to select based on a specific point.
...
#important to pay attention to the layering within ggplot() 
...
#ESRI delorme is a different base map 
