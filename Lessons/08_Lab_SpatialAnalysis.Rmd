...
upper_nuese_huc8 <- st_read("./Data/Spatial/NCHUC8.shp") %>%
  filter(SUBBASIN == "Upper Neuse")

#Create a map

mapview(huc8_sf, col.regions = "orange", alpha.regions =1)+ mapview(upper_nuese_huc8, col.regions="purple") 

# Use col.regions or fill to change the entire polygon colors, in this case color = was giving me only the borders changing. 
```
...
huc8_wind <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor')) %>%
    mutate(HUC_8 = substr(HUC_12,1,8)) %>%  #substrate command 
    group_by(HUC_8) %>%
  summarize(sumwind = sum(AvgWindEnergy))

#needs to be read in using (read.csv) or else you will not be able to summarize 

#Join to HUC_8 features

huc8_wind_sf <- merge(x = huc8_sf, 
                      y = huc8_wind, 
                      by.x = "HUC_8", 
                      by.y = "HUC_8")
...
mapview(huc8_wind_sf, zcol = "sumwind")
...
unique(huc8_wind_sf$DWQ_Basin)

# use to list the unique values 

#Summarize on DWQ Basin value

huc2_basin <- huc8_wind_sf %>%
  group_by(DWQ_Basin) %>%
  summarise(Total.Acres = sum(ACRES), Total.SQ_Miles = sum(SQ_MILES))

#dissolve is done by the group_by() function 


#Map the data
mapview(huc8_basin, zcol = "Total.Acres")
